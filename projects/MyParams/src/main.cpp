...
// colors-per-vertex to show Gaussian
std::vector<std::array<double, 3>> Gaussian_COLORS;
...
void functionCallback() {
    ...
    if (ImGui::Button("Gaussian")) {
        VertexData<double> gaussianCurvature(*mesh);
        for (Vertex v : mesh->vertices()) {
            gaussianCurvature[v] = geometry->vertexGaussianCurvature(v);
            //std::cout << gaussianCurvature[v]<<std::endl;
        }
        polyscope::SurfaceScalarQuantity* QS = psMesh->addVertexScalarQuantity("Gaussian", gaussianCurvature);
        QS->setEnabled(true);
        checkerboard->setEnabled(false);
